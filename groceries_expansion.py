...
with open(filepath, 'r') as inventory_file:
    writer = csv.reader(inventory_file, delimiter= ',', )
    for row in writer:
        print(','.join(row))
...
with open(filepath, 'r') as inventory_file:
    writer = csv.DictReader(inventory_file, delimiter = ',')
    for row in writer:
        row = dict(row)
        #print(row)
...
product_data_frame = pd.read_csv(filepath)
attributes = product_data_frame.columns.tolist()
